...
function availableVersions() {
    ...
    .then(function(versions) {
        versions = _.filter(versions, function(v) {
            return tags.isValid(v.version);
        });
        if (versions.length == 0) throw new Error("No valid version on the NPM registry");
        ...
        return {
            ...
            tags: _.chain(versions)
                ...
                .omit(function(tagVersion, tagName) {
                    console.log(tagVersion, tagName);
...
function resolveVersionLocally(version) {
    ...
    version = _.chain(versions)
        .find(function(v) {
            return tags.satisfies(v.version, version);
        })
        .value();
...
function requireVersion(version) {
    ...
    version = _.find(versions, function(gb) {
        return tags.satisfies(gb.version, version);
    });
...
function getVersion(version) {
    ...
    .then(function(resolved) {
        ...
        if (!gitbook) throw "GitBook Version "+resolved+" is corrupted";
        return gitbook;
    });
}
